...
// Only load config file if environment variables are not set
let currentConfig = {};
try {
  ...
  console.log(
    "No config file found or invalid format. Using environment variables."
  );
}
...
module.exports = {
  ...
  // Fetch port number
  getPort: () => process.env.PORT || currentConfig.port || 5001,
  ...
  getPrivateEmailService: () => ({
    ...
    password:
      ...
      currentConfig.privateEmail?.password,
    secure:
